...
PATCHLEVEL = 3
SUBLEVEL = 4
EXTRAVERSION = Constqian
U_BOOT_VERSION = $(VERSION).$(PATCHLEVEL).$(SUBLEVEL)$(EXTRAVERSION)
VERSION_FILE = $(obj)include/version_autogenerated.h # "="  变量值从后面找，":=" 从前面找，这个文件是自动生成的
#host是主机，arch 架构的缩写，表示主机架构
HOSTARCH := $(shell uname -m | \             # `` 反引号 里面写命令可以执行命令 var=`pwd` ，等价于 var=$(shell pwd) ，得到i686 cpu型号
	sed -e s/i.86/i386/ \                    # | 竖线是管道  把前一步的输出作为后一步的输入 sed 字符串改变  “.” 用来匹配 此次匹配6 
	    -e s/sun4u/sparc64/ \
	    -e s/arm.*/arm/ \
	    -e s/sa110/arm/ \
	    -e s/powerpc/ppc/ \
...
export	HOSTARCH HOSTOS #导出的是环境变量

# Deal with colliding definitions from tcsh etc.
VENDOR=

#########################################################################
# Allow for silent builds 静默编译不显示打印信息 编译时 make -s 
...
OBJTREE		:= $(if $(BUILD_DIR),$(BUILD_DIR),$(CURDIR))# 编译出的.0文件的目录
SRCTREE		:= $(CURDIR)								#源码目录
TOPDIR		:= $(SRCTREE)								#
LNDIR		:= $(OBJTREE)
...
ifndef CROSS_COMPILE #定义交叉工具编译链前缀，不同CPU架构前缀不一样，后缀一样，make传参
ifeq ($(HOSTARCH),$(ARCH))
CROSS_COMPILE =
else
ifeq ($(ARCH),ppc)
